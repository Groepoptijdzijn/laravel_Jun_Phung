...
<body>
	<div class="welcome">
		...
		<h1>Blade has arrived in {{ $theLocation }} <br/> on {{ "--|--|--" }}.</h1>
		@if ($theWeather == 'everything')
			<p>It's raining everything!</p>
		@elseif ($theWeather == 'stormy')
			<p>Go outside and die for me</p>
		@else
			<p>There is no forecast available, and it's probably raining everything.</p>
		@endif
		<p></p>
			<ul style="text-align:left">
                @foreach($theLandmarks as $landmark)
                    @unless($landmark == 'Amsterdam')
                        <li>{{ $landmark }}</li>
                    @endunless
                @endforeach
			</ul>
